...
const analyzeWithCustom = async (prompt: string): Promise<string> => {
    ...
    // Construct full URL with proxy if present
    ...
    try {
        ...
        if (error.message === 'Failed to fetch') {
            throw new Error(`Connection Failed to ${finalUrl}. \nCheck: \n1. Is the server running? \n2. Is CORS enabled? \n3. If using an external API (Perplexity/OpenAI) from the browser, try adding a CORS Proxy in settings.`);
        }
        throw error;
    }
};
...
const chatWithCustom = async (messages: ChatMessage[], context?: string): Promise<string> => {
    ...
    try {
        ...
        if (error.message === 'Failed to fetch') {
            throw new Error(`Connection Failed to ${finalUrl}. \nPossible causes: \n1. Server not running. \n2. CORS blocked by browser (Try adding a CORS Proxy in Settings). \n3. Mixed Content (Using HTTP local server from HTTPS app).`);
        }
        throw error;
    }
};
